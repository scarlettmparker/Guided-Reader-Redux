...
public:
  AnnotationHandler(ConnectionPool &connection_pool) : pool(connection_pool)
  {
    ...
    conn->prepare("select_annotation_data",
    ...
    conn->prepare("update_annotation",
                  "UPDATE public.\"Annotation\" "
                  "SET description = $1 "
                  "WHERE id = $2");

    conn->prepare("delete_annotation_interactions",
                  "DELETE FROM public.\"UserAnnotationInteraction\" "
                  "WHERE annotation_id = $1");

    conn->prepare("delete_annotation",
